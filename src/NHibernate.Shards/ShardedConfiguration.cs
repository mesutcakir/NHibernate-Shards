	...
	public class ShardedConfiguration
    {
            ...
            Dictionary<short, short> virtualShardToShardMap)
        {
            ...
            if (virtualShardToShardMap.Count != 0)
            {
                ...
                this.shardToVirtualShardIdMap = new Dictionary<short, ICollection<ShardId>>();

                foreach (var pair in virtualShardToShardMap)
                {
                    // see if we already have a set of virtual shards
                    if (!this.shardToVirtualShardIdMap.ContainsKey(pair.Value))
                    {
                        // we don't, so create it and add it to the map
                        this.shardToVirtualShardIdMap.Add(pair.Value, new HashSet<ShardId>());
                    }
                    var set = this.shardToVirtualShardIdMap[(pair.Value)];
        ...
        public IShardedSessionFactory BuildShardedSessionFactory()
        {
            ...
            foreach (IShardConfiguration config in shardConfigs)
            {
                ...
                if (!this.shardToVirtualShardIdMap.TryGetValue(shardId, out var hashKeys))
                {
                    hashKeys = new HashSet<ShardId> { new ShardId(shardId) };
                }

                sessionFactories.Add(BuildSessionFactory(), hashKeys);
